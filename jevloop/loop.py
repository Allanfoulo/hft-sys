...
def _read_account(alpaca) -> dict:
    """Return the Alpaca account values used by the dashboard.

    The broker account is the source of truth for equity and cash. A
    dashboard refresh failure is recorded without interrupting the trading
    loop or fabricating a balance.
    """
    try:
        raw = alpaca.get_account()
    except AlpacaAPIError as exc:
...
def _write_latest(
    symbol, block, ticks, meta, started_at, api_error_streak, account=None
) -> None:
    calls = len(ticks)
    ...
    last_route = meta.get("route") or next(
        (t.get("route") for t in reversed(ticks) if t.get("route")), None
    )
    last_model = meta.get("model") or next(
        (t.get("model") for t in reversed(ticks) if t.get("model")), None
    )
    payload = {
